...
def run_snapshot_main(snapshot, out_dir, render_only=True):
    """Invoke render.main() from a normalized snapshot without credentials."""
    snapshot_file = Path(out_dir).parent / "snapshot.json"
    snapshot_file.write_text(json.dumps(snapshot), encoding="utf-8")
    argv = ["render.py", "--snapshot-file", str(snapshot_file),
            "--out-dir", str(out_dir)]
    if render_only:
        argv.append("--render-only")
    with mock.patch.object(sys, "argv", argv), \
            mock.patch.object(render, "gql",
...
class SnapshotInput(unittest.TestCase):
    def test_render_only_uses_normalized_snapshot_without_network(self):
        with tempfile.TemporaryDirectory() as td:
            out = Path(td) / "out"
            run_snapshot_main(public_snapshot(), out)
            for name in ("stats.svg", "streak.svg", "languages.svg",
                         "external.svg"):
                self.assertTrue((out / name).exists())
            self.assertIn(">1</text>",
                          (out / "external.svg").read_text(encoding="utf-8"))
    ...
    def test_render_only_reports_missing_required_section(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            snapshot_file.write_text(json.dumps(public_snapshot()),
                                     encoding="utf-8")
            with mock.patch.object(render.data, "load_snapshot",
                                   return_value=missing), \
                    mock.patch.object(sys, "argv", [
                        "render.py", "--snapshot-file", str(snapshot_file),
                        "--render-only", "--out-dir", str(Path(td) / "out")]), \
                    mock.patch.dict(os.environ, {}, clear=True):
                with self.assertRaisesRegex(
                        ValueError,
                        r"snapshot missing required section: pull_requests"):
                    render.main()
