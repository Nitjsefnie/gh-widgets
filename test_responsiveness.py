...
class SnapshotInput(unittest.TestCase):
    def test_render_only_uses_normalized_snapshot_without_graphql(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            snapshot_file.write_text(json.dumps(public_snapshot()),
                                     encoding="utf-8")
            out = Path(td) / "out"
            argv = ["render-responsiveness.py", "--snapshot-file",
                    str(snapshot_file), "--render-only", "--out-dir", str(out)]
            with mock.patch.object(sys, "argv", argv), \
                    mock.patch.object(resp, "gql",
                                      side_effect=AssertionError("network called")), \
                    mock.patch.dict(os.environ, {}, clear=True):
                resp.main()
            self.assertTrue((out / "responsiveness.svg").exists())
    ...
    def test_render_only_reports_missing_required_section(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            snapshot_file.write_text(json.dumps(public_snapshot()),
                                     encoding="utf-8")
            with mock.patch.object(resp.data, "load_snapshot",
                                   return_value=missing), \
                    mock.patch.object(sys, "argv", [
                        "render-responsiveness.py", "--snapshot-file",
                        str(snapshot_file), "--render-only", "--out-dir",
                        str(Path(td) / "out")]), \
                    mock.patch.dict(os.environ, {}, clear=True):
